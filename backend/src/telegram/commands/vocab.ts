...
export async function handleGetVocabulary(bot: TelegramBot, msg: TelegramBot.Message, dateString?: string) {
  ...
  // Get quiz for target date
  const dailyQuiz = await DailyQuiz.findOne({ date: targetDate });

  if (!dailyQuiz) {
    bot.sendMessage(chatId,
      `⏰ Không có từ vựng cho ngày ${targetDate}.\n\n` +
  ...
  const dateLabel = targetDate === getTodayDateString() ? 'hôm nay' : targetDate;
  let message = `📚 Từ vựng ngày ${dateLabel} - ${dailyQuiz.topicName}\n\n`;
  message += `Có ${Math.min(10, vocabularyList.length)} từ:\n\n`;
